...
CI tip: For the frontend container, use `npm run start:serve` (static build + serve) to minimize memory and avoid orchestrator timeouts or SIGKILL (137). If you run `npm start` under CI (CI=true), it will auto-redirect to `start:serve`. `start:ci` and `start:lowmem` are available if you specifically need the webpack dev server.
